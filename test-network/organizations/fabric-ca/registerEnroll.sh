#!/bin/bash
function createOrg1 {
  infoln "Enrolling ca_org1 admin"
  ...
  cp "${PWD}/organizations/fabric-ca/tls-ca/ca-cert.pem" "${PWD}/organizations/peerOrganizations/org1.example.com/tlsca/tlsca.org1.example.com-cert.pem"
...
#   cp "${PWD}/organizations/fabric-ca/org1/ca-cert.pem" "${PWD}/organizations/peerOrganizations/org1.example.com/msp/tlscacerts/ca.crt"
   # C
#opy org1's CA cert to org1's /tlsca directory (for use by clients)
#   mkdir -p "${PWD}/organizations/peerOrganizations/org1.example.com/tlsca"
...
#   set -x
#   fabric-ca-client register --caname ca-org1 --id.name user1 --id.secret user1pw --id.type client --id.attrs "verifier=true" --tls.certfiles "${PWD}/organizations/fabric-ca/org1/ca-cert.pem"
#   { set +x; } 2>/dev/null
...
#   infoln "Enrolling the TLSCA Admin"

#   mkdir -p organizations/tls-ca
#   export FABRIC_CA_CLIENT_TLS_CERTFILES=${PWD}/organizations/fabric-ca/tls-ca/ca-cert.pem
#   export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/tls-ca

#   set -x
#   fabric-ca-client enroll -u https://tls-ca-admin:tls-ca-adminpw@localhost:7052
#   { set +x; } 2>/dev/null
  
#   infoln "Registering peer0 org1"
#   set -x
#   fabric-ca-client register --id.name peer0-org1 --id.secret peer0pw --id.type peer -u https://localhost:7052 
#   { set +x; } 2>/dev/null

#   infoln "Registering peer0 org2"
#   set -x
#   fabric-ca-client register --id.name peer0-org2 --id.secret peer0pw --id.type peer -u https://localhost:7052 
#   { set +x; } 2>/dev/null
  
#   infoln "Registering orderer"
#   set -x
#   fabric-ca-client register --id.name orderer --id.secret ordererpw --id.type orderer -u https://localhost:7052
#   { set +x; } 2>/dev/null
# }
